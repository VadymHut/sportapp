...
const staffFilters = [<SearchInput key="q" source="q" alwaysOn placeholder="Search…" />];
...
const ConfirmDeleteBody = () => {
  ...
  return (
    <Box sx={{ pt: 0.5 }}>
      <Typography sx={{ mb: 1 }}>
        You're about to delete staff member <b>{rec.name} {rec.surname}</b>. You will not be able
        to restore it.
      </Typography>
      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.75, mb: 0.5 }}>
        {rec.jobTitle && (
          <Chip size="small" label={`Job title: ${rec.jobTitle}`} variant="outlined" />
        )}
        <Chip
          size="small"
          label={`Personal code: ${rec.personalCode ?? '-'}`}
          variant="outlined"
        />
      ...
      <Box sx={{ width: '100%', maxWidth: MAX_WIDTH, mx: 'auto' }}>
        <Card variant="outlined" sx={{ width: '100%' }}>
          <CardContent sx={{ pt: 2.1, pb: 0 }}>
            ...
            <Box sx={{ mb: 1 }}>
              <FilterForm filters={staffFilters} />
            </Box>
            ...
            <Datagrid
              ...
              sx={{
                ...
                '& .column-jobTitle': { whiteSpace: 'nowrap' },
